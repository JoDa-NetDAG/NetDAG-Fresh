(() => {
  ...
  function saveAnchoredRecordLocally(recordId, hashValue) {
    const records = loadLocalRecords();
    const existingIndex = records.findIndex((item) => item.recordId === recordId);

    const recordData = {
  recordId,
  hash: hashValue,
  productImage: document.getElementById("provCompanyLogo")?.getAttribute("src") || "",
savedAt: new Date().toISOString()
  ...
  function printCertificate() {
    ...
    const companyValue = certificateClone.querySelector("#provOutCompany");
const companyLogoData = document.getElementById("provCompanyLogo")?.src || "";

if (
  companyValue &&
  companyLogoData &&
  companyLogoData !== window.location.href &&
  !companyLogoData.endsWith("/")
) {
  const logo = document.createElement("img");
  logo.setAttribute("src", companyLogoData);
  ...
  function copyFullCertificate() {
    ...
    const lines = [
      ...
      "",
      "Generated by NetDAG Provenance • NetDAG Provenance Network"
    ];
  ...
  async function copyVerificationLink() {
    ...
    const verifyUrl =
      `${window.location.origin}${window.location.pathname}?id=${encodeURIComponent(recordId)}#prov-mvp-demo`;
...
const blob = new Blob(
  ...
  async function startQrScanner() {
    ...
    els.scanQrBtn.textContent = "Scanning QR...";
    ...
    try {
      ...
      await qrScannerInstance.start(
        ...
        async (decodedText) => {
          ...
          const match = String(recordId).match(/NDG-PROV-[A-Z0-9-]+/i);
          if (match) {
            recordId = match[0];
          }
  ...
  async function anchorProduct(recordId, hashValue) {
    try {
      ...
      try {
        saveAnchoredRecordLocally(recordId, hashValue);
      } catch (saveErr) {
...
if (productPhoto) {
  if (meta?.productPhoto) {
    ...
    if (els.certVerifiedOn) {
      els.certVerifiedOn.textContent = formatTimestamp(data.storedTimestamp);
    }

    if (els.outId) {
      els.outId.textContent = data.storedRecordId || "—";
    }

    if (els.outHash) {
      els.outHash.textContent = data.storedHash || "—";
    }

    if (els.outTime) {
      els.outTime.textContent = formatTimestamp(data.storedTimestamp);
    }
    ...
    if (window.QRCode && document.getElementById("provQrCode")) {
      ...
      if (qrBox) {
        ...
        const verifyUrl =
          `${window.location.origin}${window.location.pathname}?id=${encodeURIComponent(data.storedRecordId)}#prov-mvp-demo`;
    ...
    setStatus(
      richMetaAvailable
        ? "Verified successfully on NetDAG Provenance Network."
